if (window.内部バージョン === "3") {
  (function suppressInitialTransitionAndDark(){
    ...
    if (typeof バージョン === "undefined") {window.バージョン = "2.06";}//過去バージョンで、アプリ本体でバージョン定義があるため、二重定義を避ける(アプリ本体はconstで定義されている)
...
if (残り問題番号.length === 0) {
  ...
  return;
     ...
     if (!フラッシュカード) {
      ...
      let mark_タイトル = `過去の正解割合です。${過去出題}問問中${過去正解}問正解(${Math.round(過去正解/過去出題*100)}%)`;
...
function getWordMark(word, stats) {
  ...
  if (!stats[word] || !stats[word].total) return ""; // null 安全
