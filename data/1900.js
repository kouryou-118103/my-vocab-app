if (window.内部バージョン === "3") {
  (function suppressInitialTransitionAndDark(){
    ...
    if (typeof バージョン === "undefined") {var バージョン = "2.08.3";}//過去バージョン(バージョン2以前)で、アプリ本体でバージョン定義があるため、二重定義を避ける(アプリ本体はconstで定義されている)
...
document.addEventListener("click", function(event) {
  const dialog = document.querySelector(".update-dialog");
  if (dialog && !dialog.querySelector("div")?.contains(event.target)) {
    dialog.remove();
  }
});
