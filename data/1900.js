if (window.内部バージョン === "3") {
  (function suppressInitialTransitionAndDark(){
    ...
    if (typeof バージョン === "undefined") {var バージョン = "2.10.5";}//過去バージョン(バージョン2以前)で、アプリ本体でバージョン定義があるため、二重定義を避ける(アプリ本体はconstで定義されている)
...
document.addEventListener("keydown", function(event) {
  if (!document.getElementById("settings").classList.contains("hidden") && event.key === "Enter"){
    startQuiz()
    return;
  }
