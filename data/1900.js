if (window.内部バージョン === "3") {
  (function suppressInitialTransitionAndDark(){
    ...
    if (typeof バージョン === "undefined") {var バージョン = "2.09.7";}//過去バージョン(バージョン2以前)で、アプリ本体でバージョン定義があるため、二重定義を避ける(アプリ本体はconstで定義されている)
...
function showNextQuestion() {
  ...
  // 通常モードまたは「いやらしいモード」
  let options = generateOptions(正解);
...
function generateOptions(正解) {
  let options = [正解];

  if (いやらしいモード) {
    const index1900 = 英語.indexOf(正解) !== -1 ? 英語.indexOf(正解) : 日本語.indexOf(正解);
    const startIdx = Math.max(0, Math.min(出題語句ABC.length - numChoices * 2, 出題語句ABC.indexOf(ABCnum[index1900]) - Math.floor(numChoices)));
    const endIdx = startIdx + numChoices * 2;
    const 選択肢ABC用 = 出題語句ABC.slice(startIdx, endIdx);

    const optionsSet = new Set(options);
    while (optionsSet.size < numChoices) {
      const randChoice = 選択肢ABC用[Math.floor(Math.random() * 選択肢ABC用.length)];
      const choice = 出題方向 === "ja-en" ? 英語[ABCnum.indexOf(randChoice)] : 日本語[ABCnum.indexOf(randChoice)];

      if (!optionsSet.has(choice)) {
        optionsSet.add(choice);
        options.push(choice);
      }
    }
  } else {
    options = [...options, ...選択肢範囲.slice(0, numChoices - 1)];
  }
